...
void diagnostics_dump_correlator_plan_td_stats(FILE *f, const struct correlator_plan_td *plan)
{
	...
	fprintf(f, "instrument A: phase centre = (%g, %g, %g)\n", gsl_vector_get(plan->baseline->instrument_a->phase_centre, 0), gsl_vector_get(plan->baseline->instrument_a->phase_centre, 1), gsl_vector_get(plan->baseline->instrument_a->phase_centre, 2));
	fprintf(f, "instrument A: n = %d\n", plan->proj_a->n);
	fprintf(f, "instrument A: l max = %d\n", plan->proj_a->l_max);
	fprintf(f, "instrument B: phase centre = (%g, %g, %g)\n", gsl_vector_get(plan->baseline->instrument_b->phase_centre, 0), gsl_vector_get(plan->baseline->instrument_b->phase_centre, 1), gsl_vector_get(plan->baseline->instrument_b->phase_centre, 2));
